...
2. `packets.py:normalize_outcome()` reads the timestamp and session lineage from the raw record, redacts the content, and fingerprints the kind plus redacted result only (dates and session identifiers are excluded from the fingerprint). Each packet carries a normalized, bounded session lineage identifier. Validation errors are fixed safe messages without caller data, and `write_runtime_artifacts()` validates/redacts again before appending external runtime artifacts.
...
4. `improvement.py:evaluate_packets()` counts distinct sessions per fingerprint. It emits a suggestion for a critical kind on any occurrence, or for a repeated signal only when the same fingerprint appears in at least three distinct sessions whose span (latest minus earliest) is at most seven days and whose latest occurrence is within seven days of the current time. Results are suggestion-only and require human review; they are not approvals or applied changes.
